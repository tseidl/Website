            ...
            prior = NULL)
  {
    ...
    if (attr(termobj, "response") == 1) {
      ...
      if (group == TRUE) {
        ...
        if (!(posint(K) && max(K) <= stmobj$settings$dim$K))
          stop(
            "Topics specified as response in formula must be a set of positive integers equal to or less than the number of topics in the model."
          )
      }
    ...
    for (i in 1:nsims) {
      if (thetatype == "None")
        ...
        thetasims <- thetaPosterior(stmobj,
                                    nsims = 1,
                                    type = thetatype,
                                    documents = documents)
      ...
      for (k in K) {
        if (group == TRUE) {
          ...
          grouped_thetasims <-
            apply(thetasims[, topics_to_group], 1, sum)
        ...
        if (group == TRUE & i == 1)
          K_grouped[k] <-
          paste0(c(k, ": ", paste0(topics_to_group, collapse = "+")), collapse = "")
      }
    }
    toreturn <-
      list(
...
qr.lm <- function(y, qx) {
  if (length(y) != nrow(qx$qr)) {
    ...
    if (length(y) != (nrow(qx$qr) - ncol(qx$qr)))
      stop("number of covariate observations does not match number of docs")
...
summary.qr.lm <- function (object) {
  ...
  mss <-
    ifelse(attr(z$terms, "intercept"), sum((f - mean(f)) ^ 2), sum(f ^ 2))
  rss <- sum(r ^ 2)
...
extract.estimateEffect <-
            ...
            frexw = 0.5)
  {
    ...
    simbetas <-
      stm:::simBetas(parameters = x$parameters, nsims = nsims)
    ...
    out <- lapply(topics, function(i) {
      ...
      if (method == "difference") {
        ...
        out_inner <- data.frame(
          ...
          topic = i,
          covariate = covariate,
          covariate.value = paste0(cov.value1,
                                   "-", cov.value2),
          estimate = mean(diff),
          std.error = sd(diff),
          ci.level = ci.level,
          ci.lower = quantile(diff,
                              offset),
        ...
        out_inner <- data.frame(
          ...
          estimate = rowMeans(sims),
          std.error = sqrt(apply(sims,
                                 1, stats::var)),
          ci.level = ci.level,
          ci.lower = apply(sims,
                           1, quantile, probs = offset),
          ci.upper = apply(sims,
                           1, quantile, probs = (1 - offset)),
          label = labels[which(x$topics ==
                                 i)]
        )
        out_inner["tval"] <-
          out_inner["estimate"] / out_inner["std.error"]
        rdf <- nrow(x$data) - length(out_inner["estimate"])
        out_inner["p"] <-
          2 * stats::pt(abs(out_inner[["tval"]]), rdf, lower.tail = FALSE)
      }
